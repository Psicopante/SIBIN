...
import { obtenerDescargos } from "@/services/actasService";
...
const ActasDescargo = () => {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Tooltip title="Imprimir acta">
          <IconButton
            onClick={() => navigate(`/descargos/${params.row.numeroActa}/print`)}
            sx={{
  ...
  return (
    <Box m="20px">
      <Header title="Actas de descargo" subtitle="Actas de descargo de los empleados." />
      <Card sx={{ p: 2, mt: 1, backgroundColor: colors.backGround[100] }}>
        {loading ? (
          <Box textAlign="center" py={4}>
            <CircularProgress color="secondary" />
            <Typography mt={2}>Cargando actas...</Typography>
          </Box>
        ) : error ? (
